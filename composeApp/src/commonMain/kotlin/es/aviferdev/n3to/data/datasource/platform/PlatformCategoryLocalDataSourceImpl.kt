...
class PlatformCategoryLocalDataSourceImpl(
    ...
    override fun getByCategoryIncludingArchived(assetCategoryId: String): Flow<List<Platform>> =
        queries.selectByCategoryIncludingArchived(assetCategoryId).asFlow().mapToList(Dispatchers.IO)
            .map { list -> list.map { it.toDomain() } }
    ...
    override suspend fun link(platformId: String, assetCategoryId: String): Result<Unit> = runCatching {
        withContext(Dispatchers.IO) {
            queries.link(platformId, assetCategoryId, Clock.System.now().toEpochMilliseconds())
        }
    }

    override suspend fun unlink(platformId: String, assetCategoryId: String): Result<Unit> = runCatching {
        withContext(Dispatchers.IO) {
