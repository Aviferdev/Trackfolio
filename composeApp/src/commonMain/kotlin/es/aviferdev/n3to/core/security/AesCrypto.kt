...
object AesCrypto {
    ...
    private fun aesDecryptBlock(block: ByteArray, w: Array<IntArray>): ByteArray {
        ...
        for (round in 13 downTo 1) {
            invShiftRows(state); invSubBytes(state); addRoundKey(state, w, round); invMixColumns(
                state
            )
        }
    ...
    private fun shiftRows(state: Array<IntArray>) {
        for (r in 1..3) {
            val row = IntArray(4) { state[r][(it + r) % 4] }; row.copyInto(state[r])
        }
    }

    private fun invShiftRows(state: Array<IntArray>) {
        for (r in 1..3) {
            val row = IntArray(4) { state[r][(it - r + 4) % 4] }; row.copyInto(state[r])
        }
    }
    ...
    private fun gm2(b: Int): Int =
        if (b and 0x80 != 0) (b shl 1) xor 0x1b and 0xFF else (b shl 1) and 0xFF
    ...
    private fun gm(b: Int, n: Int): Int {
        ...
        var acc = 0
        while (m > 0) {
            if (m and 1 != 0) acc = acc xor r; r = gm2(r); m = m shr 1
        }
        return acc
    }
